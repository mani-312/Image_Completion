...
parser.add_argument('--device_ids', nargs = "+", type = int, help = "Device ids for dataparallel")
...
def main(args):
    ...
    gpu = torch.device(args.device_ids[0])
    ...
    if args.data_parallel:
        model_cn = DataParallel(model_cn,device_ids = args.device_ids)
    ...
    if args.init_model_cd is not None:
        model_cd.load_state_dict(torch.load(
            ...
            map_location='cpu'))
    if args.data_parallel:
        model_cd = DataParallel(model_cd, device_ids = args.device_ids)
