...
# Use it for comparision with constext encoder
def gen_input_mask_center(
        shape, hole_size, hole_area=None):
    """
    * inputs:
        - shape (sequence, required):
                Shape of a mask tensor to be generated.
    ...
    * returns:
    ...
    for i in range(bsize):
        ...
        for _ in range(n_holes):
            # choose patch width
            mask[i, :, int(mask_h/4): int(mask_h/4)+int(mask_h/2) ,int(mask_h/4): int(mask_h/4)+int(mask_h/2)] = 1.0
    return mask

# Use it for comparision with Context Encoder
def gen_input_mask_random(
    ...
    * returns:
    ...
    for i in range(bsize):
        ...
        for _ in range(n_holes):
            # choose patch width
            start_row = np.random.randint(0,int(mask_h/2))
            start_col = np.random.randint(0,int(mask_h/2))
            mask[i, :, start_row : start_row+int(mask_h/2) ,start_col: start_col+int(mask_h/2)] = 1.0
    return mask
