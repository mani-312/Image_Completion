...
from utils import poisson_blend, gen_input_mask, gen_input_mask_random, gen_input_mask_center
...
def main(args):
    ...
    model.load_state_dict(torch.load(args.model, map_location='cpu')) # map_location refers to model will be
                                                                      #loaded to that device
    ...
    # Use to compare Context encoder
    # mask = gen_input_mask_center(
    #      shape=(1, 1, x.shape[2], x.shape[3]),
    #      hole_size=(
    #          (args.hole_min_w, args.hole_max_w),
    #          (args.hole_min_h, args.hole_max_h),
    ...
    with torch.no_grad():
        ...
        save_image(x,args.output_img+'/original.jpg')
        save_image(x_mask,args.output_img+'/Mask.jpg')
        save_image(inpainted,args.output_img+'/recon.jpg')
        save_image(imgs, args.output_img+'/grid.jpg', nrow=3)
